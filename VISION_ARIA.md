...
**UCS Integration (current state).** Aria runs a single agent loop and one planning path. An earlier flag-gated UCS intelligence layer (`src/ucs.py`, `UCS_ENABLED`) was a dormant duplicate of the agent loop and was removed. What survived is the observability-and-tuning spine: every reasoning call is logged to the `loop_executions` table (model, tokens, cost, truncation data), and the offline eval CLI (`src/eval.py`) reads it to score prompt versions by approval rate. Model configuration lives in `models.yaml`. The MCP fleet, the risk-tier confirmation mechanism, and the cost guardrails are shared by every path.
